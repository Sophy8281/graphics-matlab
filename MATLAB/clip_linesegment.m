% parametric line clipping using homogeneous co-ordinates and cohen sudherland outcodes
function [ p0, p1, is_clipped, is_removed] = clip_linesegment_CS(p0, p1)
...
RegionCodes = [1; 2; 4; 8; 16; 32; ] ; % order: left, right, bottom, top, near, far
RegionIndex = [1; 1; 2; 2; 3; 3; ];
RegionSign = [1; -1; 1; -1; 1; -1;];
is_removed = 0;
is_clipped = 0;

% outcode = FNTBRL {FAR, NEAR ... LEFT}
outcode0 = ComputeOutCodes(p0, RegionCodes); 
outcode1 = ComputeOutCodes(p1, RegionCodes);
...
while (done~=1)   
    ...
    if  not(bitor(outcode0, outcode1))  % accept        
        ...
        % start with first non zero outcode (order 0 to 1)
         ...
         elseif bitand(outcodeOut , RegionCodes(5))  
                 d1 =  p0(4) + RegionSign(5)*p0(RegionIndex(5));
                 d2  = p1(4) + RegionSign(5)*p1(RegionIndex(5));            
                 t =  d1/(d1-d2);         
                  p(1:3, :)= p0(1:3, :) + t*(p1(1:3, :) - p0(1:3, :)); 
                 p(4) = abs(p(RegionIndex(5)));  % avoid recomputing w 
                 
        elseif bitand(outcodeOut , RegionCodes(6)) 
                 d1 =  p0(4) + RegionSign(6)*p0(RegionIndex(6));
                 d2  = p1(4) + RegionSign(6)*p1(RegionIndex(6));            
                 t =  d1/(d1-d2);         
                 p(1:3, :)= p0(1:3, :) + t*(p1(1:3, :) - p0(1:3, :)); 
                 p(4) = abs(p(RegionIndex(6)));  % avoid recomputing w 
         end
        
         
       if (outcodeOut == outcode0) % update p0 and recompute new outcode
           outcode0 = ComputeOutCodes(p, RegionCodes);           
            p0 = p;
           
       else % update p1 and recompute new outcode
           outcode1 = ComputeOutCodes(p, RegionCodes); 
           p1 = p; 
       end         
    end
end